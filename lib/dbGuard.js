...
function sendDbError(res, error, fallbackMessage) {
  const knownUnavailableCodes = new Set(['P1001', 'P1002', 'DB_TIMEOUT', 'NO_DATABASE_URL']);
  const knownUnavailableNames = new Set([
  ...
  const isUnavailable = knownUnavailableCodes.has(error?.code) || knownUnavailableNames.has(error?.name);
